...
export default function App() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      /* 6 ── Horizontal scroll "How it works" — desktop only.
         On smaller screens the panels stack vertically (handled by
         Tailwind's flex-col / lg:flex-row classes below), so the
         pinned horizontal-scroll animation is scoped to lg+ via
         matchMedia, and a lightweight vertical reveal is used instead
         for mobile / tablet. */
      ...
      mm.add('(min-width: 1024px)', () => {
        ...
        return () => {
          if (hTween.scrollTrigger) hTween.scrollTrigger.kill()
          hTween.kill()
        }
      })
        ...
        gsap.utils.toArray('.h-panel').forEach(panel => {
          ...
          if (visual) {
            gsap.fromTo(visual,
              { y: 36, opacity: 0 },
              {
                y: 0, opacity: 1, duration: 0.8, ease: 'power2.out', delay: 0.12,
  ...
  return (
    <div ref={rootRef}>
      ...
      <div className="min-h-screen relative">
        ...
        {/* HERO */}
        <section className="relative min-h-screen flex items-center px-6 md:px-12 lg:px-[120px] pt-[68px] overflow-hidden">
          <div
            ref={heroBgRef}
              ...
              <div className="hero-stats flex items-center justify-center lg:justify-start gap-5 sm:gap-7 flex-wrap">
                {[['12,000+', '기업 사용 중'], ['★ 4.9', '앱스토어 평점'], ['99.9%', '서비스 가동률']].map(([val, label], i) => (
                  <div key={label} className="flex items-center gap-5 sm:gap-7">
                    ...
                    <div>
                      <div className="text-[20px] sm:text-[22px] font-extrabold text-[#f0efff] font-['Space_Grotesk']">{val}</div>
        ...
        {/* HOW IT WORKS — horizontal scroll on desktop, vertical stack on mobile/tablet */}
        <section
  id="how"
  ref={hScrollRef}
  className="
    ...
    {HOW_STEPS.map((step, i) => (
      <div
        ...
        className={`
          ...
          ${i === 0 ? 'pt-32 sm:pt-36 md:pt-44' : 'pt-20 sm:pt-24 md:pt-28'}
          lg:pt-[100px]
      ...
      />
        ...
        <section id="pricing" className="py-20 md:py-28 lg:py-[120px] px-6 md:px-12 lg:px-[120px] relative overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-stretch max-w-[980px] mx-auto">
            ...
            <div className="p-card reveal reveal-d1 flex-1 p-8 sm:p-10 rounded-[28px] relative border border-brand/35 bg-brand/[0.08] backdrop-blur-2xl">
